...
import { api } from '../api'
import { displayReason, displayRisk, displayValue, formatDateTime } from '../localization/display'
...
async function load() {
  ...
  try {
    ...
    error.value = e?.message ? displayReason(e.message) : '风险中心加载失败'
...
async function check() {
  ...
  try {
    ...
    await load()
  } catch (e: any) { error.value=e?.message ? displayReason(e.message) : '风险检查失败' }
  finally { loading.value=false }
}
...
async function resolve(row: any) {
  try {
    ...
    await load()
  } catch (e: any) { error.value=e?.message ? displayReason(e.message) : '处理失败' }
}
...
<template>
  ...
  <div class="risk-layout">
    <div class="card"><h3>未处理风险事件</h3><div v-for="row in openEvents" :key="row.id" class="event"><div><b :class="row.level==='HIGH'?'down':''">{{ row.symbol }} · {{ displayValue(row.event_type) }} · {{ displayRisk(row.level) }}</b><p>{{ row.message }}</p><small>现价 {{ row.current_price || '--' }} / 触发线 {{ row.trigger_price || '--' }} / {{ formatDateTime(row.event_time) }}</small></div><button class="btn secondary" @click="resolve(row)">标记处理</button></div><div v-if="!openEvents.length" class="muted empty">当前没有未处理风险</div></div>
  ...
  <div class="card history"><h3>已处理风险记录</h3><table class="table"><thead><tr><th>股票</th><th>类型</th><th>级别</th><th>说明</th><th>触发时间</th><th>处理时间</th></tr></thead><tbody><tr v-for="row in resolvedEvents" :key="row.id"><td>{{ row.symbol }}</td><td>{{ displayValue(row.event_type) }}</td><td>{{ displayRisk(row.level) }}</td><td>{{ row.message }}</td><td>{{ formatDateTime(row.event_time) }}</td><td>{{ formatDateTime(row.resolved_at) }}</td></tr></tbody></table></div>
</template>
