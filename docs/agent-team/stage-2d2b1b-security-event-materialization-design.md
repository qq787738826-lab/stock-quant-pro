...
状态：**本任务分支已完成证券状态事件物化契约冻结，等待 GitHub 独立审查及合并；2D-2B-1B-1 实现尚未开始。**
...
- `recordNamespace`；
- `source`；
- `sourceVersion` 所属的来源契约身份；
- `sourceInstrumentId`；
- `securityLogicalKey`；
- mapping contract/version 与 append-only 审计字段。

数据库必须拒绝同一有效 mapping key 指向多个稳定身份。mapping 的更正、合并或拆分必须新增可审计版本，不能覆盖旧关系。
...
→ 恰好一条 terminal processing attempt
→ 恰好一条 normalization result
→ 0 或 1 个 SECURITY_STATUS_EVENT_V1
```
...
| outcome | event 数量 | 语义 |
|---|---:|---|
| `EVENT_MATERIALIZED` | 1 | 本次事务首次创建逻辑 event 与唯一 lineage |
| `EVENT_REUSED` | 1（既有） | 已存在完全相同 event；本次 result 引用并逐字段验证后复用 |
| `NO_STATE_CHANGE` | 0 | 身份已解析，但 resultingState 与 predecessor 相同 |
| `IDENTITY_UNRESOLVED` | 0 | 缺少 sourceInstrumentId 或显式 mapping |
| `UNSUPPORTED_CONTRACT` | 0 | 需要 V2、多字段或局部 valid 更正，V1 不支持 |
| `CONFLICT` | 0 | 同一逻辑 revision/身份/predecessor 下出现不一致内容或竞争结果 |
| `PROJECTION_FAILED` | 0 | 事件投影输入准备或冻结校验失败；本阶段不写 history |
...
- 必须进入 Manifest V2；
- accepted/rejected 统计规则必须由 V2 固定向量冻结，不能在封存时临时解释。
...
冻结的迁移方式是为列使用常量 `NOT NULL DEFAULT` 完成既有行保守赋值，随后 `DROP DEFAULT`，再添加约束。不得通过普通业务 `UPDATE` 回填，不得禁用 V6 不可变 trigger。`securityLogicalKey` 和 `eventLogicalKey` 若不能仅凭既有权威事实可靠生成，必须通过单独迁移设计解决，不能猜测或按 symbol 自动合并。
...
- 一个 terminal attempt 恰好一个 result；
...
- `NO_STATE_CHANGE` 等无事件结果不得引用 event；
...
- retry 可引用既有 raw/event，但必须产生自身 terminal attempt/result 审计；
...
- V2 封存必须根据实际数据库集合重算并验证 Manifest V2。
...
- raw → result → 0/1 event 基数；
- outcome 与 nullable event 字段组合；
...
### 18.2 2D-2B-1B-0：security event contract freeze（本任务）

只冻结本文契约与权威状态，不创建迁移或生产实现。完成后仍不能宣称 event 物化已实现。

### 18.3 2D-2B-1B-1：TEST/DEMO event materialization foundation（未开始）

只有本文通过独立审查并合并后才能开始。实现 identity mapping 基础、normalization result、V1 event 物化、event lineage 和 Manifest V2；FORMAL 仍关闭。
...
阶段 2D-2B-1B-0 只完成证券状态事件物化契约冻结。后续 1B-1 必须在 TEST/DEMO 边界内，严格复用 V1 event 主表与转换语义，以显式稳定身份、唯一 normalization result、唯一 event lineage 和 Manifest V2 建立可审计闭环。
