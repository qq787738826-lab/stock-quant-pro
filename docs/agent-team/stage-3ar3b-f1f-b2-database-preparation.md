...
- 管理员创建受限角色和数据库、收紧 PUBLIC、创建专用 Schema 和 `btree_gist`；管理员连接关闭并清零秘密后才读取最终专用用户密码，再以专用用户执行主 V1—V13。
...
- PostgreSQL `16.13` 全新随机实例：`15 / 0 / 0 / 0`；验证角色、数据库、Schema、PUBLIC 收紧、精确主 V1—V13、无 baseline、无治理 history/V14/验收表、零事实、目标复用拒绝及零残留，核心数据库测试 `Skipped=0`。
