...
合并前增量审查发现 `c8342830` 已绑定 Schema owner，但数据库 owner 与迁移对象 owner 尚未纳入正式回读。本次最小修复将数据库 owner 绑定为专用角色，隔离 PostgreSQL 15+ 的 `pg_database_owner` 对 `public` Schema 的隐式所有权，并对数据库、Schema及全部非扩展迁移关系/过程对象执行owner回读；扩展成员通过`pg_depend`单独识别，不冒充业务迁移对象。
...
- 管理员创建受限角色，并将全新专用数据库 owner 精确绑定为该角色后收紧 PUBLIC；一次性 bootstrap 专用用户创建自身专用 Schema 和 `btree_gist`，管理员连接关闭并清零秘密后才读取最终专用用户密码，再以专用用户执行主 V1—V13。完成前回读数据库、Schema、迁移关系对象及过程对象 owner 均为专用角色。
...
- PostgreSQL `16.13` 全新随机实例：`15 / 0 / 0 / 0`；验证专用角色拥有数据库、Schema及V1—V13迁移对象，PUBLIC 收紧、精确主V1—V13、无baseline、无治理history/V14/验收表、零事实、目标复用拒绝及零残留，核心数据库测试`Skipped=0`。
