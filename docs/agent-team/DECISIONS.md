...
41. **AGENT_CONTEXT_2G_V1 与公告覆盖资格**：只有团队规则版本 `1.4.0-stage-2g-announcement-risk-v1` 选择 `AGENT_CONTEXT_2G_V1/SECURITY_EVENTS_CONTEXT_V1`，同时复用 2F backtestContext 和 2H portfolioContext；旧 2B、2D-1、2E-1、2F、2H profile/contextHash 不变。securityEvents 固定回看 180 个自然日，只选择 `reportedPublishDate<=requestTradeDate` 且 `knownAt<=knowledgeCutoff` 的每个逻辑公告最新可见版本。可用性必须由同 symbol、正确来源/Provider、完整覆盖窗口、`complete=true`、不晚于 cutoff 且年龄不超过 24 小时的批次证明；完整 0 公告批次可以可用，无批次、过期、部分或范围不足必须安全不可用。研究来源、日期精度、无修订/历史完整性/PIT/正式资格和无 PDF 解析限制必须随 context 固定披露。
