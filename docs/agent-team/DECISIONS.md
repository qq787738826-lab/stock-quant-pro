...
40. **公告来源身份与 ANNOUNCEMENT_CANONICAL_V1**：来源公告身份优先使用 CNINFO 明确公告 ID，编码为 `CNINFO:<id>`；没有明确 ID 时只允许对规范化 HTTP/HTTPS URL 生成 `CNINFO_URL_SHA256:<sha256>` 并标记 `URL_DERIVED`，禁止由标题、日期、symbol 或抓取顺序生成身份。`ANNOUNCEMENT_CANONICAL_V1` 对来源、身份强度、证券、名称、标题、报告日期、日期精度、规范化 URL 和 assurance flags 使用冻结白名单、UTF-8 与 SHA-256 小写十六进制；Java 生成生产 Hash 和 observationVersion，Java/Python 通过固定输入、canonical 文本与外部固定预期值的黄金向量交叉验证。
