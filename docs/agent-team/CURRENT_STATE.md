...
- 3A-R3B-F1E 当前状态：类型化缩减研究准入、独立本地数据库身份守卫、单日 1—3 证券共享会话、每证券 `daily/adj_factor/trade_cal` 精确三请求、全批次单事务捕获及内存公式级 QFQ 已在任务分支实现。当前 `REDUCED_RESEARCH_ROUTE_DECISION=DEDICATED_LOCAL_RESEARCH_PATH`、`REDUCED_RESEARCH_LOCAL_RUNTIME_IMPLEMENTATION_READY=true`、`REDUCED_RESEARCH_CONTROLLED_ACCEPTANCE_READY=true`，但真实受控运行尚未由用户接受，因此 `REDUCED_RESEARCH_OPERATIONAL_READY=false`。该阶段待 ChatGPT 基于实际 Git 提交验收，尚未合入；不改变完整 F1 技术阻断、生产/正常业务库、scheduler、Agent/回测、F2B/F3 或交易门禁。
...
- F1E 的结果均为 Codex 本地执行证据，不是 GitHub Actions CI：8 个 core、179 个 server 生产源码完成干净 Java 编译；F1E 新增定向 `16/0/0/0`；F1A—F1E、Provider V2 与 QFQ 联合回归 `132/0/0/0`，其中 Provider V2 10 项、QFQ 权威引擎 19 项及 18 个黄金向量不变；`quant-core` 全量 `4/0/0/0`；命令级排除全部 `*IntegrationTest/*Postgres*/*CrossLanguage*/*Live*` 的 `quant-server` 安全全量 `423/0/0/0`。全新的 PostgreSQL 16.13 临时实例中，F1A `4/0/0/0`、F1C `3/0/0/0`、F1E `6/0/0/0`，均 `Skipped=0`；F1E 覆盖 1/2/3 证券精确请求、Provider 或捕获失败全事务回滚、search path TOCTOU、幂等和 append-only。测试后 F1A/F1C 随机 Schema、F1E 专用 Schema、55432 监听和临时目录残留均为 0，F1E 临时 public 表数仍为 0，测试内 public 指纹前后相同。全部数据来自合成 Gateway，F1E 新增 Provider 请求为 0，Tushare 累计仍为 20，iFinD 为 0；未读取 Token，也未访问既有或正常业务数据库。
