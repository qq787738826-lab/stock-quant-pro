...
- 专用测试库存在本任务开始前已经保留的验收数据。部分数据库测试在清理自身任务后断言五张业务表全局为零，因此在非空测试库上失败；这不是本次生产链路回归，但测试隔离假设需要后续治理，不能通过删除既有数据或弱化断言掩盖。
- 最近验收：Python 33/33 通过；无数据库变量 Java 智能体运行 133 项（8 项跳过、0 项失败）；无变量 quant-server 运行 134 项（8 项跳过、0 项失败）；真实 Python 冒烟通过；Vue 生产构建通过；本地端到端闭环通过。带既有数据的数据库环境下 Java 智能体 138 项中 8 项因全表零假设失败、1 项跳过。
