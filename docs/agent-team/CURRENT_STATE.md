...
- 当前集成分支 HEAD：`e3777602fadd65f3af0a2ba8ac6e886693d745d5`
...
- 3A-R3B-F1F-B2-RUNNER 当前状态：任务分支 `codex/1.4.0-stage-3ar3b-f1f-b2-controlled-runner` 基于 `e3777602fadd65f3af0a2ba8ac6e886693d745d5` 修复上述安全门。合并前最终安全审查又收紧了显式 Jackson 模块、输出审计结束顺序、Provider 组件关闭、精确主/治理 Flyway history、单 JAR classpath、不可覆盖的 `JarLauncher`、隔离 commit 构建及 Java 运行时绑定，并扩展 PostgreSQL 零 DDL 反例。任务分支仍待基于最终实际 Git 提交复验，尚未合入。真实 B2、真实三请求、正式构建证明和正式授权均未执行；合入后仍需基于新的冻结集成 SHA 重新完成 B2-PRE。
...
- 当前正式状态：`F0_AUDIT_RESULT=PARTIAL`、`FREE_IMPLEMENTATION_PATH=RESEARCH_PREVIEW_FIRST`、`FREE_PRODUCT_PREVIEW_GATE=PASS`、`FREE_PROVIDER_VALIDATION_GATE=BLOCKED`、`PAID_PROVIDER_UPGRADE_DECISION=PENDING`、`IFIND_TRIAL_ACTIVATION_GATE=BLOCKED`。Track A 的免费研究预览产品形态验证已经完成；该 PASS 不改变 Provider、PIT、效果、Shadow、付费数据或交易资格。F1A/F1B/F1C/F1D/F1E、F1F-A 与 F1F-B1 均已验收合入；F1E 的专用本地实现与受控验收准备 READY 不等于 operational、生产或正常业务库运行 READY。F1D 已把当前个人研究书面许可闭环为 PASS，但完整技术合同仍有十项阻断，因此 `F1_ENTRY_READINESS=BLOCKED_TECHNICAL_EVIDENCE`、`fullF1EntryReady=false`。F1F-B2-RUNNER 只在任务分支修复执行与冻结安全门，尚未合入，也未执行真实受控验收，当前 `CONTROLLED_ACCEPTANCE_STATUS=NOT_RUN`、`REDUCED_RESEARCH_OPERATIONAL_READY=false`。Tushare 累计真实业务请求为 20（B1 为 10、F1A 为 10，其余后续阶段均为 0），iFinD 真实调用数为 0；正常业务库 V13 未执行，F2B/F3、3A-R3B-1 均未开始，Day 002 未创建，scheduler 关闭，3B 未开始。
...
**在智能体规则能力工作线上，3A-R3B-0、免费优先治理规划、F0 审计、F0.5 双轨治理、F2A/R1/R1A/R1B、产品门治理、Track B0、Track B1、F1A、F1B、F1C、F1D、F1E、F1F-A 与 F1F-B1 均已验收并合入；当前集成 HEAD 为 `e3777602fadd65f3af0a2ba8ac6e886693d745d5`。F0 最终结论为 `F0_AUDIT_RESULT=PARTIAL`。Track A 已完成，当前 `FREE_PRODUCT_PREVIEW_GATE=PASS`。Track B 主要路线为 Tushare Pro，备用为 iFinD。B1 十项受控业务请求均 PASS；完整证据探针仍为 `PARTIAL_NOT_COMPLETE`、其执行时点法律前置为 `NOT_MET`。F1D 已把当前个人研究书面门闭环为 PASS，再分发、商业数据服务和 Token 共享仍为 `NOT_GRANTED`。F1E 已合入专用本地研究数据库守卫与 1—3 证券单日原子批次，但仅有 `REDUCED_RESEARCH_LOCAL_RUNTIME_IMPLEMENTATION_READY=true` 和 `REDUCED_RESEARCH_CONTROLLED_ACCEPTANCE_READY=true`，`REDUCED_RESEARCH_OPERATIONAL_READY=false`。F1F-A 与 F1F-B1 只准备并加固受控验收机制，真实验收仍为 `NOT_RUN`；F1F-B2-RUNNER 尚在任务分支。当前 `F1_ENTRY_READINESS=BLOCKED_TECHNICAL_EVIDENCE`，完整技术资格仍有十项阻断。** Day 001 仍只有 1 个有效观察日和 3 个 item。Tushare 保持 `V13_LINEAGE_PARTIAL/PIT_PARTIAL`、稳定证券 ID `PARTIAL`，不声明完整公司行动、Provider PIT 或永久身份资格。四项正式门禁仍为 `PASS/BLOCKED/PENDING/BLOCKED`。Tushare 累计真实业务请求为 20（B1 10、F1A 10，F1B/F1C/F1D/F1E/F1F-A/F1F-B1/F1F-B2-RUNNER 0），iFinD 为 0，scheduler 关闭，Day 002 未创建。Codex 不得自行升级 operational/生产运行、开始 F2B/F3、激活 iFinD、恢复长期观察、开始 3B 或其他阶段。
