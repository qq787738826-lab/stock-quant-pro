...
阶段 2D-1、2D-2A、2D-2B-1A 和文档阶段 2D-2B-1B-0 完成不等于完整阶段 2D、完整阶段 2D-2 或完整阶段 2D-2B 完成；这些上位阶段仍处于进行中。已完成内容是基础设施、只读事实上下文、数据质量门禁、受限的当前证券池宽度状态规则、时态事实基础、来源无关摄取基础及事件物化契约，不代表 event 已物化，也不代表完整六智能体分析、完整市场环境模型、历史无前视市场宽度、真实股票分析或投资建议能力已经完成。
...
当前真实可用的是 Java 权威任务和持久化、阶段 2A 第一批四类上下文、确定性技术指标、确定性 DATA_QUALITY 门禁、`marketBreadth` 只读事实、`scanResult` 历史扫描事实、阶段 2D-1 受限的当前证券池宽度状态规则、V6 时态事实基础、V7 来源无关摄取基础、Hash 与 JSONB 稳定往返、缺数安全降级、失败原子性和 Vue 工作台观察能力。V7 当前只提供 TEST/DEMO 可审计 ingestion 基础，FORMAL 继续由数据库门禁拒绝；当前不存在 V8，尚未实现 identity registry 或物化证券状态 event，尚未形成 history/calendar projection 或 Universe。除 DATA_QUALITY 和该受限宽度状态规则外，其余四个专业智能体规则仍未实现；当前仍不具备完整六智能体分析、完整 MARKET_REGIME、TECHNICAL_ANALYSIS 真实规则、STRATEGY_BACKTEST 解释、公告风险、持仓风险、投资建议或自动交易能力。
...
DATA_QUALITY 为 PASS 或 WARN 且宽度规则可执行时，MARKET_REGIME 可为 `COMPLETED`、decision为 `WARN`，但总控仍为 `finalDecision=INSUFFICIENT_DATA`、继承 gate、`score=0`、`confidence=0`；BLOCKED 时仍为 `BLOCKED_BY_DATA_QUALITY`。只有 `POSITION_RISK` 在满足冻结契约时才有正式否决权。其余四个专业智能体仍返回未实现的 `INSUFFICIENT_DATA/NOT_APPLICABLE` 安全结果；总控不是第七个run。阶段 2D-1 不构成完整 MARKET_REGIME、完整股票分析或投资建议。

## 数据库、前端与本地运行

- 数据库 Schema 当前已升级至 Flyway V7。V6 新增 dataset 版本、不可变证券状态事件、双时间证券状态历史和 SSE/SZSE 版本化交易日历；V7 新增来源无关 ingestion run、security/calendar raw、run-record 关联、terminal attempt、retry、namespace、assurance、封存与 Manifest V1。V6/V7 均不回填现有 `securities` 或 `daily_bars`。
...
当前智能体团队阶段禁止新增接入或宣称已具备：实时行情查询、新 AKShare 查询、公告上下文、真实或新增模拟持仓上下文、LLM/付费 API、超出阶段 2D-1 确定性宽度score的完整评分策略、交易写操作、自动下单和券商控制。不得编造价格、指标、证据或投资结论。阶段 2D-1 的score不构成完整 MARKET_REGIME、牛熊判断、收益预测、投资建议或交易信号。
...
- 本地专用测试库 public Schema 存在 V6 checksum 与当前仓库不一致的历史环境问题。2D-2B-1A 未执行 Flyway repair/clean、未修改 public，并通过随机 Schema 隔离完成验收；该问题属于独立环境治理事项，不能被描述为 V7 功能失败，也不能在后续任务中被静默修复或掩盖。
- 正式证券状态来源、交易日历来源、许可、本地持久化/历史回放权利及稳定 source instrument ID 尚未批准，因此 FORMAL/PIT 摄取继续阻断。

## 下一阶段唯一入口

**阶段 2D-2B-1B-1：TEST/DEMO event materialization foundation。**

完整阶段 2D、完整阶段 2D-2 和完整阶段 2D-2B 仍处于进行中。阶段 2D-2A、2D-2B-1A 与文档阶段 2D-2B-1B-0 已完成；下一步只能按已冻结契约实现 TEST/DEMO identity mapping、normalization result、V1 event 物化、唯一 event lineage 和 Manifest V2。

2D-2B-1B-1 尚未开始，且只允许 TEST/DEMO；2D-2B-1B-2 approved source adapter 仍被外部来源和许可决策阻断。当前仍无 V8、identity registry、证券 event 物化、history/calendar projection、历史 universe 快照、PIT 行情版本、公司行动治理或 `marketBreadth V2`；无前视历史回放和评测集也尚未建立。阶段 2E 仍未开始。阶段 2D-2B 禁止外部行情补数、LLM 权威决策、投资建议和交易写操作。
