...
- 当前集成分支 HEAD：`1ad039038829f6e752ce6ecea6da0a3d88882df7`
...
- 3A-R3B-F1B 当前状态：只使用 Tushare 官方资料完成完整技术合同与缩减路线判定；类型化 `TechnicalClaim(status,evidenceIds)` 模型生成 `TUSHARE_TECHNICAL_ROUTE_DECISION=REDUCED_RESEARCH_ONLY`、`TUSHARE_REDUCED_RESEARCH_CONTRACT=READY`、`fullTechnicalContractReady=false`、`reducedResearchContractReady=true`。其中 `READY` 只表示合同定义完成；`QFQ_FORMULA_QUALIFICATION=VERIFIED`，但 `QFQ_OPERATIONAL_RUNTIME_QUALIFICATION=PARTIAL`、`REDUCED_RESEARCH_RUNTIME_READY=false`，现有 `QfqAsOfEngine` 仍保留公司行动 lineage 门禁。官方总表的 200 次/分钟与 `stock_basic=50`、`daily=500` 次/分钟接口页证据形成 `OFFICIAL_ENDPOINT_RATE_LIMITS=PARTIAL_CONFLICT_IDENTIFIED`，当前未实施 Endpoint 级限流。raw/factor/calendar 满足缩减合同；公司行动、Provider revision/旧版本、永久证券身份和全历史 `DAILY_EXACT` 不满足完整 F1。代码、测试和治理文档已在任务分支完成，待 ChatGPT 基于实际 Git 提交验收，尚未合入；下一运行实现仍需用户单独授权。本阶段 Provider 新增调用为 0，未检查 Token，未访问数据库。
...
- 当前正式状态：`F0_AUDIT_RESULT=PARTIAL`、`FREE_IMPLEMENTATION_PATH=RESEARCH_PREVIEW_FIRST`、`FREE_PRODUCT_PREVIEW_GATE=PASS`、`FREE_PROVIDER_VALIDATION_GATE=BLOCKED`、`PAID_PROVIDER_UPGRADE_DECISION=PENDING`、`IFIND_TRIAL_ACTIVATION_GATE=BLOCKED`。Track A 的免费研究预览产品形态验证已经完成；该 PASS 不改变 Provider、PIT、效果、Shadow、付费数据或交易资格。F1A 有限个人用途 Adapter 已验收合入；F1B 已在任务分支冻结缩减技术合同，但缩减运行入口和 Endpoint 级限流均未实现，尚待实际 Git 验收、合入及下一阶段单独授权。Tushare 累计真实业务请求为 20（B1 为 10，F1A 为 10），iFinD 真实调用数为 0；正常业务库 V13 未执行，完整 F1 仍同时受书面许可与技术证据阻断，F2B/F3、3A-R3B-1 均未开始，Day 002 未创建，scheduler 关闭，3B 未开始。
...
**在智能体规则能力工作线上，3A-R3B-0、免费优先治理规划、F0 审计、F0.5 双轨治理、F2A/R1/R1A/R1B、产品门治理、Track B0、Track B1 与 F1A 均已验收并合入；集成 HEAD 为 `1ad039038829f6e752ce6ecea6da0a3d88882df7`。F0 最终结论为 `F0_AUDIT_RESULT=PARTIAL`。用户于 `2026-07-29 16:44 +08:00` 明确认可产品形态，当前 `FREE_PRODUCT_PREVIEW_GATE=PASS`，Track A 正式完成。Track B 主要路线为 Tushare Pro，备用为 iFinD。用户已开通 Tushare 2000 积分；`2026-07-30` 十项 B1 受控业务请求均 PASS，最低成本和最小 raw/factor/calendar/身份/dividend/`DAILY_EXACT` 技术样例阻断已解除。完整证据探针仍为 `PARTIAL_NOT_COMPLETE`，其历史法律前置为 `NOT_MET`，不要求重跑。此后官方企业微信只书面确认“可以用来当量化数据来源”，个人本地长期存储、回测和智能体分析三项 Provider 书面许可仍为 `UNVERIFIED`；用户另行明确授权有界个人实现并承诺不分发、转售或商业化原始数据。F1A 五 Endpoint Adapter、显式 `MANUAL_BOUNDED`、类型化有限 FORMAL 捕获、隔离验证与阶段 10 次受控联调已经验收合入。F1B 类型化技术合同在任务分支判定 `TUSHARE_TECHNICAL_ROUTE_DECISION=REDUCED_RESEARCH_ONLY`、`TUSHARE_REDUCED_RESEARCH_CONTRACT=READY`，但 `QFQ_OPERATIONAL_RUNTIME_QUALIFICATION=PARTIAL`、`REDUCED_RESEARCH_RUNTIME_READY=false`，且 Endpoint 级限流未实现；合同任务尚待实际 Git 验收和合入，运行实现还需下一阶段单独授权。当前 `F1_ENTRY_READINESS=BLOCKED_MULTIPLE`，由 `BLOCKED_WRITTEN_PERMISSION` 与 `BLOCKED_TECHNICAL_EVIDENCE` 组成。** Day 001 已形成 1 个真实受控 Shadow 批次和 3 个 item，均以 `BLOCKED_BY_DATA_QUALITY` 安全终结并完成正式人工复核；后续受控行情更新形成的 780 条 V9 观察全部 `sourceRevision=NULL`。当前 Tencent 来源资格仍是 `PROVIDER_REVISION_UNVERIFIED`，`version=18` 不得作为 revision。F0 没有找到可单独承担完整 V13/QFQ lineage 的免费来源。Tushare 当前也仍为 `V13_LINEAGE_PARTIAL/PIT_PARTIAL`，稳定证券 ID 为 `PARTIAL`；F1A/F1B 不声明完整公司行动、Provider PIT 或永久身份资格。`FREE_PRODUCT_PREVIEW_GATE=PASS` 与 `FREE_PROVIDER_VALIDATION_GATE=BLOCKED` 相互独立。Tushare 累计真实业务请求为 20（B1 10、F1A 10），F1B 新增为 0，iFinD 调用数为 0，scheduler 关闭，Day 002 未创建。Codex 不得自行开始受控本地研究摄取、F2B/F3、激活或调用 iFinD、恢复长期观察、开始 3B 或其他阶段。

阻断项包括正式证券状态来源、稳定 source instrument ID、完整公司行动、revision 语义以及 published/effective 时间语义。当前免费聚合源和 `securities` 当前态投影均不得被视为正式来源；2G 的研究级 AKShare/CNINFO 公告来源同样不得用于解除这些门禁。当前仍未实现完整正式 source adapter、`PROVIDER_PIT_VERIFIED`、`SECURITY_STATUS_EVENT_V2`、`security_status_history` 正式投影、完整正式 Provider trading calendar、Universe snapshot、`MARKET_BREADTH_V2`、完整 MARKET_REGIME、公告 PDF 语义分析或生产扫描切换。F1A 只通过类型化专用入口增加 `PROVIDER_CAPTURE/RESEARCH_ONLY/formalEligible=false` 的 Tushare raw/factor/calendar 有限个人用途路径，通用捕获仍拒绝 FORMAL；F1B 只把该路线收敛为 `REDUCED_RESEARCH_ONLY`，这不等于完整 FORMAL 资格。3A-R3B-0 的 raw/factor/calendar/action、2F V2 和 Shadow 已作为 TEST/DEMO 离线能力合入，但不能替代真实来源资格；正常业务库尚未执行 V13。阶段 2F、2G、2H、2I、3A-1、3A-R1、3A-R3A、3A-R3B 规划、3A-R3B-0、免费优先治理规划、F0 审计、F0.5、F2A/R1/R1A/R1B 产品链和 F1A 均已完成并合入；产品形态门当前为 PASS，F1B 待实际 Git 验收，完整 F1 仍同时受书面许可与技术证据阻断，受控本地研究摄取、F2B、F3、3A-R3B-1 至 R3B-3 均未开始。当前只有 1 个有效观察日和 3 个 shadow item，尚未达到 20 个有效观察日、200 个 item、主要原因人工复核和正式观察报告门槛，因此完整 3A 未完成，3B 未开始。阶段 2D-2B 禁止外部行情补数、LLM 权威决策、投资建议和交易写操作。
