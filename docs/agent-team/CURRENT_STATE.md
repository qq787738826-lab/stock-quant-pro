...
- 3A-R3B-F1A 当前状态：Tushare 官方企业微信于 `2026-07-30` 书面回复“问：这个可以用来当量化数据来源吧；答：可以”，只支持 `WRITTEN_QUANT_DATA_SOURCE_USE_PERMISSION=VERIFIED`；个人本地长期存储、回测和 Agent 三项书面许可均为 `UNVERIFIED`。用户已明确 `USER_PERSONAL_USE_IMPLEMENTATION_AUTHORIZATION=CONFIRMED`，有限个人实现为 `APPROVED_BY_USER`，并承诺不分发、转售或商业化原始数据。Adapter 已收紧为默认 `DISABLED`、显式 `MANUAL_BOUNDED` 五 Endpoint、10 次共享会话预算、单进程 180 次/分钟和每 API 90000 次/日安全限制；时间序列 Endpoint 使用固定两日范围，stock_basic/dividend 参考数据分别在 DTO 前执行 1/1000 行硬上限。通用捕获继续拒绝 FORMAL，只有类型化 Tushare 有限个人授权入口可捕获 raw/factor/calendar；capability 明示完整 F1 未就绪且 Provider 书面许可未完成。初始 6 请求与修复剩余 4 请求均完成且零重试；待 ChatGPT 基于实际 Git 增量提交验收，尚未合入。`F1_ENTRY_READINESS=BLOCKED_MULTIPLE`，精确包含 `BLOCKED_WRITTEN_PERMISSION` 和 `BLOCKED_TECHNICAL_EVIDENCE`；`V13_LINEAGE_PARTIAL`、`PIT_PARTIAL`、稳定证券 ID `PARTIAL` 不变。
...
- 当前正式状态：`F0_AUDIT_RESULT=PARTIAL`、`FREE_IMPLEMENTATION_PATH=RESEARCH_PREVIEW_FIRST`、`FREE_PRODUCT_PREVIEW_GATE=PASS`、`FREE_PROVIDER_VALIDATION_GATE=BLOCKED`、`PAID_PROVIDER_UPGRADE_DECISION=PENDING`、`IFIND_TRIAL_ACTIVATION_GATE=BLOCKED`。Track A 的免费研究预览产品形态验证已经完成；该 PASS 不改变 Provider、PIT、效果、Shadow、付费数据或交易资格。Tushare 累计真实业务请求为 20（B1 为 10，F1A 为 10），iFinD 真实调用数为 0；正常业务库 V13 未执行，F1A 仅限个人用途实现待验收且尚未合入，完整 F1 仍同时受书面许可与技术证据阻断，F2B/F3、3A-R3B-1 均未开始，Day 002 未创建，scheduler 关闭，3B 未开始。
