...
- 3A-R3B-F1B 当前状态：只使用 Tushare 官方资料完成完整技术合同与缩减路线判定；类型化资格模型生成 `TUSHARE_TECHNICAL_ROUTE_DECISION=REDUCED_RESEARCH_ONLY`、`TUSHARE_REDUCED_RESEARCH_CONTRACT=READY`、`fullTechnicalContractReady=false`、`reducedResearchContractReady=true`。raw/factor/calendar 与请求结束日锚定的研究级 QFQ 满足缩减合同；公司行动、Provider revision/旧版本、永久证券身份和全历史 `DAILY_EXACT` 不满足完整 F1。代码、测试和治理文档已在任务分支完成，待 ChatGPT 基于实际 Git 提交验收，尚未合入；本阶段 Provider 新增调用为 0，未检查 Token，未访问数据库。
...
- 当前正式状态：`F0_AUDIT_RESULT=PARTIAL`、`FREE_IMPLEMENTATION_PATH=RESEARCH_PREVIEW_FIRST`、`FREE_PRODUCT_PREVIEW_GATE=PASS`、`FREE_PROVIDER_VALIDATION_GATE=BLOCKED`、`PAID_PROVIDER_UPGRADE_DECISION=PENDING`、`IFIND_TRIAL_ACTIVATION_GATE=BLOCKED`。Track A 的免费研究预览产品形态验证已经完成；该 PASS 不改变 Provider、PIT、效果、Shadow、付费数据或交易资格。F1A 有限个人用途 Adapter 已验收合入；F1B 缩减技术合同已在任务分支冻结，尚待实际 Git 验收和合入。Tushare 累计真实业务请求为 20（B1 为 10，F1A 为 10），iFinD 真实调用数为 0；正常业务库 V13 未执行，完整 F1 仍同时受书面许可与技术证据阻断，F2B/F3、3A-R3B-1 均未开始，Day 002 未创建，scheduler 关闭，3B 未开始。
