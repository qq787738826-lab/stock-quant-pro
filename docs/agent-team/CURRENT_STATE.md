...
- 当前集成分支 HEAD：`08943b4f6af03c75aa4df2a4ecf2494bede4e57b`
...
- 3A-R3B-F2A 当前状态：`/research-preview` 免费研究预览产品的前端技术实现与 Codex 本地离线验证已在任务分支完成，待 ChatGPT 基于实际 Git 提交验收，尚未合入，用户视觉验收尚未进行。页面只读复用既有 GET API，并严格隔离 `EXISTING_RESEARCH_SNAPSHOT/RESEARCH_HISTORICAL_UNVERIFIED` 与 `TEST_DEMO_EXPLICIT`；未新增 Java 接口或任何外部数据调用。
...
双轨路线。轨道 A 的 F2A 已由用户单独授权：`/research-preview` 的前端技术实现已经在
当前任务分支完成，但尚待 ChatGPT 对实际 Git 提交验收、尚未合入，也尚未进行用户视觉
验收。页面只读使用既有扫描/Agent GET API，严格隔离
...
进入 F1、F2B 与 F3。F2A 的页面可见性不得被写成 Provider 资格、PIT、准确率、收益或
Shadow 证据。F1、F2B、F3 和 3A-R3B-1 均未开始，Day 002 未创建，scheduler 关闭，
3B 未开始。
