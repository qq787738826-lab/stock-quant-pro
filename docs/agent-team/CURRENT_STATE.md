...
- 3A-R3B-F1F-B1 当前状态：初始提交 `e0dfba061a1b2e335c2f0db9bc9efeac012d75c8` 的合并前安全审查发现 V14 隔离、状态机、构建证明、输出隔离、提交后 typed fact 回读和 PASSED 重验需要加固；最小修复、离线全量及新建 PostgreSQL 16.13 临时实例验证已在任务分支 `codex/1.4.0-stage-3ar3b-f1f-b1-controlled-acceptance-executor` 完成，待 ChatGPT 对最终实际 Git 提交复验，尚未合入。V14 使用独立 location 与独立治理历史，默认主历史仍为 V1—V13；执行器不属于 Spring/Controller/Runner/scheduler 入口，TEST 成功只形成候选。F1F-B2 真实验收未运行，当前 `CONTROLLED_ACCEPTANCE_STATUS=NOT_RUN`、`REDUCED_RESEARCH_OPERATIONAL_READY=false`。
