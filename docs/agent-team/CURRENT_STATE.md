...
当前真实可用的是任务基础设施、契约校验、PostgreSQL 只读事实查询、四类上下文冻结、Java 确定性技术指标、Hash 与 JSONB 稳定持久化、确定性 DATA_QUALITY 门禁、缺数据安全降级、失败原子性和工作台观察能力。除 DATA_QUALITY 外其余五个专业规则仍未实现，当前仍不具备完整智能体股票分析或投资建议能力。
...
阶段 2C 的唯一允许范围是从现有 PostgreSQL 接入 `marketBreadth`、`scanResult` 和 `backtestContext` 三类只读研究上下文；必须先审计现有业务表及字段语义，不得使用外部数据补数，不得让 Python 直连业务数据库，不得实现后续智能体评分、LLM 或交易写操作。
