...
`2026-07-30`，Tushare 官方企业微信精确书面回复为“问：这个可以用来当量化数据来源吧；
答：可以”。F1A 因而只把量化数据来源用途和用户个人实现授权更新为：
...
该后续书面回复没有逐项确认本地长期存储、回测或 Agent。用户授权个人自用实现且承诺
不分发、转售或商业化原始数据；不得将其描述为 Provider 逐项许可。它不改变 B1 完整证据
探针的历史状态：
`TRACK_B_FULL_EVIDENCE_PROBE_STATUS=PARTIAL_NOT_COMPLETE`、自动探针与响应留存书面许可仍为
`UNVERIFIED`；服务到期留存仍未验证，原始数据再分发未获授权。F1A 只实现有限个人用途
raw/factor/calendar、普通身份与 dividend 部分证据 Adapter；完整公司行动、版本、永久
身份和全历史 `DAILY_EXACT` 继续受技术证据阻断；本地长期存储、回测和 Agent 的
Provider 逐项书面许可仍受 `BLOCKED_WRITTEN_PERMISSION` 阻断。F1A 有限实现获批不改变
完整 F1 的两类阻断。
