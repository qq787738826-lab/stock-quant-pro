...
1. 对象实例内一次性授权绑定验收 ID、代码基线、Tushare、一个证券、一个交易日、三个固定
   Endpoint、三次请求、零重试、专用数据库/用户/Schema/V13、有效期和全部禁止阶段；
2. 基线、范围和有效期先于数据库检查；数据库检查先于委托 F1E 运行；同一授权对象在首次合法
   委托前以 CAS 原子消费，重复或并发使用同一对象不得再次委托；
...
F1F-A 不声称具备同 ID 重建、JVM 重启或跨进程的持久化防重。未来 F1F-B 在真实调用前必须增加
持久化 acceptance ID 唯一预占和终结；必须从最终构建产物验证完整提交哈希；必须通过日志/输出
审计证明敏感信息未输出；必须回读数据库时间证据。当前候选等级分别为
`OBJECT_INSTANCE_CAS_ONLY`、`CONFIG_DECLARED_EXACT_MATCH`、`NOT_ATTESTED` 和
`databaseReadbackVerified=false`。
