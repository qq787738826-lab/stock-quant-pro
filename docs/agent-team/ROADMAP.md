...
- 证据与运行边界：FULL 判定由每种 action 独立 claim、稳定 action ID、factor/action、Provider PIT 等带 evidenceId 的强声明共同驱动；泛化证据或裸布尔值不能越级。现有 `QfqAsOfEngine` 继续要求公司行动 lineage，公式验证没有另开运行通道。官方总表与 Endpoint 页面频次值形成 `PARTIAL_CONFLICT_IDENTIFIED`；Endpoint 级限流和缩减运行入口在 F1B 当时尚未实现。
- 缩减合同能力：F1B 为后续受控入口冻结手工有界 raw/factor/calendar、研究级 QFQ 和首次真实捕获后的 `SYSTEM_KNOWLEDGE_PIT` 边界；普通 `stock_basic` 身份与 dividend 解释证据不属于 F1C 运行入口。F1B 当时没有运行入口；禁止 Provider PIT、完整公司行动、历史 revision、永久身份、跨 Provider QFQ、正常业务库、scheduler 和全市场采集。
...
#### 3A-R3B-F1D：Tushare 书面许可闭环与 F1 单阻断收敛（已完成并合入）
...
- 提交链：`349856ea6e9e3dc423fc1ad9115886cfc8858159` → `049c750026fa00dad70c12667fad732af07d60ce` → `01024df465afcfa34dfd4efdbef7d56d32419aa1`；已通过实际 Git 最终复验，经用户批准纯 fast-forward 合入。
...
- 专用目标：只接受本机 `stock_quant_research` 数据库与用户、唯一 `tushare_research` search path 和精确 V1—V13；Provider 前及捕获事务前后重验目标与 backend PID。public、正常业务库、通用测试库和迁移不完整目标全部拒绝。
...
#### 3A-R3B-F1F-B2-DBPREP：专用数据库准备链与冻结清单校正（已完成并合入）
...
- 悬挂恢复结果：`F1FB2_20260804_SK_POSTFIX_70E8249A333E` 已由正式恢复入口单向终结为 `INTERRUPTED / STRANDED_RUNNING_PROCESS_EXITED`；`recoveryApplied=true`，Java/脚本退出码为 `0/0`，Provider 调用为 0，旧 ID 永久不可复用。
...
- 宿主执行边界：`STOCK-QUANT-HOST-BROKER` 保持 CodexSandbox 与真实用户 Credential Manager
  隔离，使用当前用户登录时启动的固定常驻 `StockQuantLocalBroker` 承载宿主 Runner；登录动作本身
  不运行 Provider。Codex formal profile 不具备 Tushare 直连或 Task Scheduler 触发权限，只能在
  fresh heartbeat 下提交严格非敏感、短期、一次性请求并等待脱敏结果；
