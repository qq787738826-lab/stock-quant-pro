...
- Windows Credential Manager 只允许三个 Target：`StockQuant/ResearchDbPassword`、
  `StockQuant/TushareToken` 和 `StockQuant/BailianApiKey`。Codex 不得枚举凭据、读取或回传原文，也不得调用 `cmdkey`
...
- Codex 只能在 `quant-server/target/stock-quant-host-broker/requests` 写严格非敏感请求，
  ...
  `CHECK_BAILIAN_CREDENTIAL_STATUS`、`RUN_M3_AGENT_RESEARCH_SMOKE`；禁止命令文本、动态脚本路径、
...
- M3 真实 LLM smoke 只允许固定宿主 Broker 以真实用户身份启动已证明 Start-Class 的
  `TushareM3AgentResearchManualRunner`，只读使用 M1/M2，并且只可访问固定阿里云百炼
  OpenAI-compatible endpoint 与 `qwen3.7-plus`。经用户批准后单次 smoke 成本硬上限为
  CNY 5.00、retry=0、redirects=NEVER；Fake/E2E、凭据状态检查和 CodexSandbox 均不得访问百炼网络。
