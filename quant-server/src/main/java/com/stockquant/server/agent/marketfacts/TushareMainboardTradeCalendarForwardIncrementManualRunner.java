...
import com.stockquant.server.agent.marketfacts.MainboardTradeCalendarForwardIncrementSanitizedResult.Result;
import com.stockquant.server.agent.marketfacts.MainboardTradeCalendarForwardIncrementSanitizedResult.ResultFile;
...
/** Fixed calendar-only forward increment; never starts Spring or Agents. */
public final class TushareMainboardTradeCalendarForwardIncrementManualRunner {
    static final int EXIT_SUCCESS = 0;
    static final int EXIT_REJECTED = 20;
    private static final int FORMAL_PORT = 38_432;

    private TushareMainboardTradeCalendarForwardIncrementManualRunner() {
    }
    ...
    static int run(String[] args, Clock clock) {
        ...
        var progress =
                new MainboardTradeCalendarForwardIncrementService.Progress();
        try {
            ...
            resultFile = ResultFile.reserve(launch.resultFile(), failure(
                    ...
                    "MAINBOARD_TRADE_CAL_FORWARD_RUNNING", false));
            Arguments bound = launch;
            Captured<MainboardTradeCalendarForwardIncrementService.Outcome>
                    captured = launch.mode() == ExecutionMode.FAKE
                    ? fake(bound, clock, progress)
                    : formal(bound, clock, progress);
            if (!captured.auditResult().clean()) {
                throw invalid(
                        "MAINBOARD_TRADE_CAL_FORWARD_OUTPUT_AUDIT_FAILED");
            }
            resultFile.write(success(launch, commit, startedAt,
                    ...
                    progress.retryCount));
            System.out.println(
                    "MAINBOARD_TRADE_CAL_FORWARD_INCREMENT_STATUS=SUCCEEDED");
            return EXIT_SUCCESS;
    ...
    private static Captured<MainboardTradeCalendarForwardIncrementService
            .Outcome> fake(Arguments launch, Clock clock,
        ...
        return TushareControlledAcceptanceOutputAudit
                .captureControlledProcess(registry -> {
                    char[] password = "FORWARD_CALENDAR_E2E_DB".toCharArray();
                    char[] token = "FORWARD_CALENDAR_E2E_TOKEN".toCharArray();
                    try {
    ...
    private static Captured<MainboardTradeCalendarForwardIncrementService
            .Outcome> formal(Arguments launch, Clock clock,
                             MainboardTradeCalendarForwardIncrementService
                                     .Progress progress) throws Exception {
        return TushareControlledAcceptanceOutputAudit
                .captureControlledProcess(registry -> {
    ...
    private static MainboardTradeCalendarForwardIncrementService.Outcome
    execute(Arguments launch, char[] password, char[] token, Clock clock,
            MainboardTradeCalendarForwardIncrementService.Progress progress) {
            ...
            try (var components = launch.mode() == ExecutionMode.FAKE
                ...
                var service =
                        new MainboardTradeCalendarForwardIncrementService(
                                ...
                                clock);
                return service.execute(launch.targetEndDate(),
    ...
    private static void seedFakePrerequisites(
        ...
        LocalDate initialMax = LocalDate.of(2026, 8, 27);
        LocalDate initialStart = initialMax.minusDays(89);
        components.mainboardUniverseCaptureService().capture(null, true,
                Set.of(), initialStart, initialMax, true,
                launch.gitCommit(),
                MainboardTradeCalendarForwardIncrementService
                        .PROVIDER_TIMEOUT, 0);
    }
    ...
    private static Result success(
            ...
            String commit,
            Instant startedAt,
            MainboardTradeCalendarForwardIncrementService.Outcome outcome,
            boolean auditClean,
            boolean fake,
            int retryCount
    ) {
        ...
        return new Result(
                MainboardTradeCalendarForwardIncrementSanitizedResult.VERSION,
                "SUCCEEDED", outcome.action(), launch.executionId(), commit,
                startedAt, outcome.completedAt(), launch.targetEndDate(),
                outcome.currentSseMaxCalDate(),
                outcome.currentSzseMaxCalDate(), outcome.startDate(),
                outcome.finalSseMaxCalDate(), outcome.finalSzseMaxCalDate(),
                outcome.latestCommonCompletedOpenTradeDate(),
                outcome.rangeCalendarDateCount(), snapshot.universeVersion(),
                snapshot.snapshotId(), snapshot.memberFingerprint(),
                snapshot.memberCount(), outcome.duplicateCount(),
                outcome.providerCalls(),
                outcome.calendarCallCountsByExchange().getOrDefault(
                        "SSE", 0),
                outcome.calendarCallCountsByExchange().getOrDefault(
                        "SZSE", 0), 0, 0, 0, retryCount,
                launch.networkRecoveryBudget(),
                launch.maximumProviderRequests(), outcome.batchIds(),
                outcome.appended(), outcome.idempotent(),
                outcome.continuousCoverage(), outcome.knownAtValid(),
                outcome.firstObservedAtValid(), outcome.lineageValid(),
                outcome.providerFieldsPreserved(),
                outcome.existingFactsUnchanged(), true, true,
    ...
    private static void requireSchema(JdbcTemplate jdbc) {
        ...
        if (version == null || version != 18) {
            throw invalid(
                    "MAINBOARD_TRADE_CAL_FORWARD_SCHEMA_VERSION_INVALID");
        }
    }
    ...
    private static void validateProof(
        ...
        if (!launch.gitCommit().equals(proof.gitCommit())
                ...
                : proof.mainboardTradeCalendarForwardIncrementEligible())) {
            throw invalid(
                    "MAINBOARD_TRADE_CAL_FORWARD_BUILD_PROOF_NOT_ELIGIBLE");
        }
    }
    ...
    private static void writeFailure(
        ...
        System.err.println(
                "MAINBOARD_TRADE_CAL_FORWARD_FAILURE_REASON=" + reason);
    }
    ...
    private static Result failure(
        ...
        return new Result(
                MainboardTradeCalendarForwardIncrementSanitizedResult.VERSION,
                "FAILED", "FAILED", launch.executionId(), commit, startedAt,
                completedAt, launch.targetEndDate(), null, null, null, null,
                null, null, 0, ResearchUniverseMainboard.VERSION, null, null,
                0, 0, progress.providerCalls,
                ...
                progress.appended, progress.idempotent, false, false, false,
                false, false, false, false, false, 0, 0, 0, 0, 0,
                auditClean, launch.mode() == ExecutionMode.FAKE, true, false,
                reason);
    }
    ...
    private static String safeCode(Throwable error) {
        ...
        return "MAINBOARD_TRADE_CAL_FORWARD_EXECUTION_FAILED";
    }
    ...
    record Arguments(
        ...
        static Arguments parse(String[] args) {
            ...
            for (String arg : args) {
                if (!arg.startsWith("--") || !arg.contains("=")) {
                    throw invalid(
                            "MAINBOARD_TRADE_CAL_FORWARD_ARGUMENTS_INVALID");
                }
            ...
            if (!values.keySet().equals(Set.of("result-file",
                    "execution-id", "git-commit", "target-end-date",
                    "database-port",
                    "maximum-provider-requests", "network-recovery-budget",
                    "execution-mode"))) {
                throw invalid(
                        "MAINBOARD_TRADE_CAL_FORWARD_ARGUMENTS_INVALID");
            }
            try {
                Arguments value = new Arguments(
                        ...
                        LocalDate.parse(values.get("target-end-date")),
        ...
        private void validate() {
            if (!executionId.matches(
                    "MBTCFWD_[0-9]{8}T[0-9]{6}Z_[A-F0-9]{12}")
