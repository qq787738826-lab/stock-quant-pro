...
$tempRoot = Join-Path $tempBase "stock-quant-f1f-b2-readback-$suffix"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'F1F_B2_READBACK_POSTGRES_TEST_FAILED'
    }
    Write-Output 'F1F_B2_READBACK_POSTGRES_TESTS=PASS'
    Write-Output "F1F_B2_READBACK_POSTGRES_PORT=$port"
} finally {
    ...
    if (Test-Path -LiteralPath $tempRoot) {
        throw 'F1F_B2_READBACK_TEMP_DIRECTORY_REMAINS'
    }
    $listenerRemaining = Get-NetTCPConnection `
        -LocalPort $port -State Listen -ErrorAction SilentlyContinue
    if ($listenerRemaining) {
        throw 'F1F_B2_READBACK_TEMP_PORT_REMAINS'
    }
    Write-Output 'F1F_B2_READBACK_TEMP_RESIDUALS=0'
}
