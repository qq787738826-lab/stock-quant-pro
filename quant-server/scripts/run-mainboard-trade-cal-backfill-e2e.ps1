...
$tempBase = [IO.Path]::GetFullPath(
    (Join-Path $env:TEMP 'stock-quant-pro-calendar-tests')).TrimEnd('\', '/')
$prefix = 'stock-quant-mainboard-calendar-e2e-'
...
$result = Join-Path $target `
    ('mainboard-calendar-e2e-' + [Guid]::NewGuid().ToString('N') + '.json')
$port = 0
$started = $false
...
function Scalar([string] $Sql) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_QUERY_FAILED'
    }
...
function Remove-Root {
    ...
    if ([IO.Path]::GetDirectoryName($full).TrimEnd('\', '/') -ne
        ...
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_CLEANUP_PATH_INVALID'
    }
...
try {
    ...
    if ((git rev-parse HEAD).Trim() -ne $ExpectedCommit -or
        ...
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_GIT_INVALID'
    }
    & "$PSScriptRoot\prepare-mainboard-trade-cal-backfill-build-proof.ps1" `
    ...
    if ($LASTEXITCODE -ne 0 -or
        ...
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_BUILD_FAILED'
    }
    ...
    $listener.Stop()
    if ($port -eq 38432) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_PORT_INVALID'
    }
    New-Item -ItemType Directory -Path $root | Out-Null
    & "$pgBin\initdb.exe" -D $data -A trust -U postgres `
        --no-locale --encoding=UTF8 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_INITDB_FAILED'
    }
    ...
    try {
        ...
        if ($process.ExitCode -ne 0) {
            throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_POSTGRES_START_FAILED'
        }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_DATABASE_SETUP_FAILED'
    }

    & "$PSScriptRoot\run-mainboard-trade-cal-backfill.ps1" `
        -ResultFile $result -ArtifactPath $artifact `
        -ExecutionId 'MBTC250_20260901T110000Z_A1B2C3D4E5F6' `
        -GitCommit $ExpectedCommit -AnchorTradeDate '2026-08-27' `
        -CalendarRangeStart '2025-04-15' `
        -CalendarRangeEnd '2026-08-27' `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_RUNNER_FAILED'
    }
    $value = Get-Content -LiteralPath $result -Raw -Encoding UTF8 |
        ConvertFrom-Json
    if ($value.status -ne 'SUCCEEDED' -or
        [int]$value.initialCommonOpenSessions -ne 60 -or
        ...
        [int]$value.sseTradeCalendarProviderCallCount -ne 1 -or
        [int]$value.szseTradeCalendarProviderCallCount -ne 1 -or
        ...
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_RESULT_INVALID'
    }
    $common = Scalar @'
SELECT count(*) FROM (
...
'@) -ne 0) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_DATABASE_INVALID'
    }
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_PACKAGED_FAKE_E2E=PASS'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_TEMP_POSTGRES_V1_V18=PASS'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_FAKE_INITIAL_COMMON=60'
    Write-Output "MAINBOARD_TRADE_CAL_BACKFILL_FAKE_FINAL_COMMON=$common"
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_FAKE_TARGET_250=PASS'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_FAKE_PROVIDER_ATTEMPTS=2'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_REAL_TUSHARE_CALLS=0'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_REAL_BAILIAN_CALLS=0'
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_BUSINESS_RUNS_CREATED=0'
} finally {
    Pop-Location
    ...
    if (Test-Path -LiteralPath $root) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_TEMP_DIRECTORY_REMAINS'
    }
    if ($port -gt 0 -and (Get-NetTCPConnection -LocalPort $port `
            -State Listen -ErrorAction SilentlyContinue)) {
        throw 'MAINBOARD_TRADE_CAL_BACKFILL_E2E_TEMP_PORT_REMAINS'
    }
    Write-Output 'MAINBOARD_TRADE_CAL_BACKFILL_TEMP_RESOURCE_RESIDUALS=0'
}
